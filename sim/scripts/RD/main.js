...
async function VisualPDE(url) {
  ...
  function init() {
    ...
    simTextures.push(simTextures[0].clone());   // [1]
    simTextures.push(simTextures[0].clone());   // [2]
    simTextures.push(simTextures[0].clone());   // [3]
    simTextures.push(simTextures[0].clone());   // [4]
    simTextures.push(simTextures[0].clone());   // [5] — additional slot for ADER-DG q2 state
    postTexture = simTextures[0].clone();
    interpolationTexture = simTextures[0].clone();

    // ADER-DG MRT (multiple render target) framebuffers. Each writes 2 outputs:
    //   * mrtPredInit:  q1_init at [0], q2_init at [1]
    //   * mrtPicard:    q1     at [0], q2     at [1]
    // Filtering and wrap mode mirror the simTextures so reads from the
    // outputs in subsequent passes have the same behaviour.
    mrtPredInit = new THREE.WebGLMultipleRenderTargets(
      options.maxDisc, options.maxDisc, 2, simTextureOpts,
    );
    mrtPicard = new THREE.WebGLMultipleRenderTargets(
      options.maxDisc, options.maxDisc, 2, simTextureOpts,
    );
    for (const t of mrtPredInit.texture) {
      t.wrapS = THREE.RepeatWrapping;
      t.wrapT = THREE.RepeatWrapping;
    }
    for (const t of mrtPicard.texture) {
      t.wrapS = THREE.RepeatWrapping;
      t.wrapT = THREE.RepeatWrapping;
    }
    ...
    // ADER-DG order 3 materials: 2 predictor inits + 2 Picard refinements +
    // 1 corrector. Their fragment shaders are assigned later (see the
    // assignFragmentShader block alongside the FE/AB2/Mid/RK4 cases).
    for (const tag of ["ADER3init1", "ADER3init2", "ADER3pic1", "ADER3pic2", "ADER3corr"]) {
      simMaterials[tag] = new THREE.ShaderMaterial({
        uniforms: uniforms,
        vertexShader: genericVertexShader(),
      });
    }
    // ADER-DG order 4 materials. Same pass structure as order 3 but the
    // shader source is generated from RDShaderDG*(4), giving 5 LGL nodes
    // per element instead of 4. Order 4 also runs an extra Picard sweep.
    for (const tag of ["ADER4init1", "ADER4init2", "ADER4pic1", "ADER4pic2", "ADER4corr"]) {
      simMaterials[tag] = new THREE.ShaderMaterial({
        uniforms: uniforms,
        vertexShader: genericVertexShader(),
      });
    }
    // ADER-DG MRT-fused materials. These compile in GLSL3 dialect so the
    // fragment shader can declare two outputs via `layout(location = N) out`.
    // One fused init pass replaces the two predictor-init passes; one fused
    // Picard pass replaces the two Picard rows.
    for (const tag of ["ADER3initMRT", "ADER3picMRT", "ADER4initMRT", "ADER4picMRT"]) {
      simMaterials[tag] = new THREE.ShaderMaterial({
        uniforms: uniforms,
        vertexShader: genericVertexShader(),
  ...
  function setSizes() {
    ...
    // DG texture layout: when ADER-DG order 3 or 4 is selected, group texels
    // into elements of size (p+1) where p is the DG polynomial degree
    // ADER-DG layout: round nXDisc DOWN to a multiple of (p+1) so each
    // x-element owns a contiguous block of (p+1) co-located LGL nodes. The
    // hybrid scheme keeps the y-axis on a uniform 4th-order central FD
    // stencil, so nYDisc is left alone.
    const dgOrderActive = aderDgOrder();
    if (dgOrderActive >= 2) {
      const nodesPerElem = dgOrderActive + 1;     // 3 for ADER3 (p=2), 4 for ADER4 (p=3)
      const elemCount = Math.max(1, Math.floor(nXDisc / nodesPerElem));
      nXDisc = elemCount * nodesPerElem;
    }
    ...
    uniforms.dgOrder.value = dgOrderActive;
    if (dgOrderActive >= 2) {
      // (p+1) texels per element, so element width is (p+1) × texel spacing.
      uniforms.u_dxElement.value = (domainWidth / nXDisc) * (dgOrderActive + 1);
    } else {
      uniforms.u_dxElement.value = 1.0;
    }
  ...
  function resizeTextures(shift = 0) {
    ...
    // ADER-DG MRT framebuffers track the same dimensions. Their contents are
    // overwritten every timestep, so we don't bother re-rendering — just
    // resize the underlying allocations.
    if (mrtPredInit) mrtPredInit.setSize(nXDisc + shift, nYDisc + shift);
    if (mrtPicard)   mrtPicard.setSize(nXDisc + shift, nYDisc + shift);
  ...
  function initUniforms() {
    ...
    uniforms = {
      ...
      // DG polynomial order (0 = FD/no DG, >=1 = DG layout active).
      // For order 1, the texture is sized so that pairs of texels (2k, 2k+1)
      // form a 2-node element of width Δx; texel 2k+1 and texel 2k+2 are
      // logically co-located duplicated boundary nodes.
      dgOrder: {
        type: "i",
        value: 0,
      },
      // DG element width (= (p+1) · texel-spacing when DG layout is active;
      // unused otherwise). Set by setSizes(). Used by the predictor and
      // corrector shaders to map reference-space derivatives to physical x.
      u_dxElement: {
        type: "f",
        value: 1.0,
      },
      // Local Lax-Friedrichs / Rusanov wave-speed magnitude used by the
      // ADER-DG corrector's boundary-lifting flux. For linear advection at
      // unit speed this is just 1; for nonlinear systems it is the local
      // max characteristic, conservatively bounded across the domain.
      dgFluxSpeed: {
        type: "f",
        value: 1.0,
      },
      // Selects which physical flux the corrector uses at element interfaces:
      //   0 = linear advection F̃(u) = -dgFluxSpeed·u (default; matches
      //       sandbox tests and the convergence verification path).
      //   1 = 2D shallow-water equations in primitive (h, u, v) form:
      //       F̃_h = -(h+H_e)·u, F̃_u = -(g·h + u²/2), F̃_v = -u·v.
      //       Uses u_dgG and u_dgHe uniforms below; max wave speed is
      //       |u| + sqrt(g·(h+H_e)) per state, taken as max over the two
      //       interface traces for the Rusanov dissipation coefficient.
      // Auto-detected from options.kineticParams in updateAderFluxConfig().
      dgFluxKind: {
        type: "i",
        value: 0,
      },
      ...
      // GL2 quadrature node uniform for the predictor-init shader (set per
      // pass to c1 ≈ 0.2113 or c2 ≈ 0.7887).
      u_c: {
        type: "f",
        value: 0.0,
      },
      // 2×2 Picard weight matrix entries (set per pass to a row of the
      // matrix [(1/4, c1−1/4); (c2−1/4, 1/4)]).
      u_aAlpha: {
        type: "f",
        value: 0.0,
      },
      ...
      // True if the active x-direction BC is periodic. The ADER-DG corrector's
      // boundary-lifting term wraps modulo nXDisc when periodic is true; on
      // non-periodic BCs it falls back to a Neumann-like trace (neighbour =
      // self) at the domain edges. Set per timestep from options.boundaryConditions_1.
      dgPeriodicX: {
        type: "bool",
        value: true,
      },
      // Constant-Dirichlet support for the ADER-DG corrector. When the user
      // selects boundaryConditions_1 == "dirichlet" AND every species' Dirichlet
      // expression parses as a finite numeric constant, dgUseDirichletX is set
      // to true and the boundary neighbour-trace uses dgDirichletXValue as a
      // fixed ghost-cell value. Expressions that depend on x, y, or t fall back
      // to the Neumann-like (neighbour = self) trace.
      dgUseDirichletX: {
        type: "bool",
        value: false,
      },
  ...
  function initGUI(startOpen) {
    ...
    controllers["timesteppingScheme"] = root
      .add(options, "timesteppingScheme", {
        ...
        "Midpoint Method": "Mid",
        "Runge-Kutta 4": "RK4",
        "ADER-DG order 1": "ADER1",
        "ADER-DG order 2": "ADER2",
        "ADER-DG order 3": "ADER3",
        "ADER-DG order 4": "ADER4",
      })
      .name("Scheme")
      .onChange(function () {
        document.activeElement.blur();
        // Switching between DG and non-DG modes changes nXDisc (the DG layout
        // groups texels into LGL elements). Re-run resize so the texture
        // sizing matches the active scheme.
        resize();
        renderIfNotRunning();
      });
  ...
  // Detect whether the user's preset matches the 2D SWE flux contract
  // (h = R, u = G, v = B) and configure the corrector's interface flux
  // accordingly. The SWE contract is identified by kineticParams that
  // declares both `g` and `H_e` as numeric constants — this is the form the
  // bundled ShallowWaterEqns / 1DLinearizedShallowWaterEqns presets use.
  // Any other kineticParams shape leaves dgFluxKind = 0 (linear advection),
  // which is the safe default that matches the convergence-test sandbox.
  function refreshAderFluxConfig() {
    const k = options.kineticParams || "";
    const gM = k.match(/(?:^|;)\s*g\s*=\s*([+-]?\d*\.?\d+(?:[eE][+-]?\d+)?)\s*(?:;|$)/);
    const hM = k.match(/(?:^|;)\s*H_e\s*=\s*([+-]?\d*\.?\d+(?:[eE][+-]?\d+)?)\s*(?:;|$)/);
    if (gM && hM) {
      uniforms.dgFluxKind.value = 1;
      uniforms.u_dgG.value = parseFloat(gM[1]);
      uniforms.u_dgHe.value = parseFloat(hM[1]);
      // Also bump the linear-advection LF-dissipation fallback so that any
      // code path still reading dgFluxSpeed sees a sensible bound.
      uniforms.dgFluxSpeed.value = Math.sqrt(
        uniforms.u_dgG.value * Math.max(uniforms.u_dgHe.value, 1e-6),
      );
    } else {
      uniforms.dgFluxKind.value = 0;
    }
  }

  // Parse the per-species Dirichlet strings as numeric constants
  // and push them into the dgUseDirichletX / dgDirichletXValue uniforms used
  // by the ADER-DG corrector. Returns true if every active species' Dirichlet
  // expression is finite-numeric (so the ghost-cell branch is safe to take);
  // any expression that depends on x, y, t, or a kineticParam falls back to
  // the Neumann-like trace.
  function refreshAderDirichletUniforms() {
    if (options.boundaryConditions_1 !== "dirichlet") {
      uniforms.dgUseDirichletX.value = false;
      return;
    }
    const strs = [
    ...
    for (let i = 0; i < options.numSpecies; i++) {
      const v = parseFloat((strs[i] || "").trim());
      if (!Number.isFinite(v) || String(v) !== (strs[i] || "").trim()) {
        uniforms.dgUseDirichletX.value = false;
        return;
      }
      parsed[i] = v;
    }
    uniforms.dgUseDirichletX.value = true;
    uniforms.dgDirichletXValue.value.set(parsed[0], parsed[1], parsed[2], parsed[3]);
  }

  // Returns the ADER-DG polynomial degree p for the active scheme, else 0.
  // Used by setSizes() to round nXDisc to a multiple of (p+1) and by the
  // shader generators to size the LGL stencil. The values are intentionally
  // one lower than the time-method order: with k Picard sweeps, the method
  // order is min(k+1, 2p), so p_ader3 = 2 still gives total order 3
  // (min(3, 4) = 3) and p_ader4 = 3 still gives total order 4 (min(4, 6) = 4).
  // Lowering p by one materially reduces visible element-boundary structure
  // for SWE-style demos without dropping the headline accuracy claim;
  // spatial truncation O(h^(p+1)) still matches the method order.
  function aderDgOrder() {
    // Visual mode swaps the ADER-DG path for the midpoint method under the
    // hood — see options.aderVisualMode. The DG layout (LGL nodes, element
    // rounding of nXDisc/nYDisc, IC projection) is bypassed when visual
    // mode is on, so this returns 0 there.
    if (options.aderVisualMode !== false) return 0;
  ...
  function timestep() {
    ...
    switch (options.timesteppingScheme) {
      ...
      case "RK4":
        ...
        break;
      case "ADER3":
        // Visual-mode short-circuit: when options.aderVisualMode is true,
        // ADER3/ADER4 dispatch to the midpoint method so the demo has the
        // FD-clean look (no element-boundary structure on screen). The
        // actual ADER-DG path runs only when visual mode is off, which is
        // what the dissertation's GLSL convergence verification uses.
        if (options.aderVisualMode !== false) {
          simDomain.material = simMaterials["Mid1"];
          uniforms.textureSource.value = simTextures[1].texture;
          renderer.setRenderTarget(simTextures[2]);
          renderer.render(simScene, simCamera);
        ...
        // Pick up the user's x-BC choice once per timestep so the
        // corrector's boundary-lifting fetch behaves correctly on non-periodic
        // problems (Neumann fallback rather than periodic wrap).
        uniforms.dgPeriodicX.value = (options.boundaryConditions_1 === "periodic");
        refreshAderDirichletUniforms();
        refreshAderFluxConfig();
        // ADER-DG order 3, MRT-fused: 5 passes → 3 passes.
        //   Pass 1 (init MRT)  : u^n → q1_init, q2_init   (mrtPredInit.texture[0/1])
        //   Pass 2 (Picard MRT): u^n, q1_init, q2_init → q1, q2  (mrtPicard.texture[0/1])
        //   Pass 3 (corrector) : u^n, q1, q2 → u^{n+1}     (simTextures[0])
        {
          uniforms.dt.value = options.dt;

          // Pass 1: fused predictor init at both GL2 nodes.
          simDomain.material = simMaterials["ADER3initMRT"];
          uniforms.textureSource.value = simTextures[1].texture;   // u^n
          renderer.setRenderTarget(mrtPredInit);
          renderer.render(simScene, simCamera);

          // Pass 2: fused Picard refinement, both rows of the 2x2 weight matrix.
          simDomain.material = simMaterials["ADER3picMRT"];
          uniforms.textureSource.value  = simTextures[1].texture;          // u^n
          uniforms.textureSource1.value = mrtPredInit.texture[0];          // q1_init
          uniforms.textureSource2.value = mrtPredInit.texture[1];          // q2_init
          renderer.setRenderTarget(mrtPicard);
          renderer.render(simScene, simCamera);

          // Pass 3: corrector → u^{n+1}.
          simDomain.material = simMaterials["ADER3corr"];
          uniforms.textureSource.value  = simTextures[1].texture;          // u^n
          uniforms.textureSource1.value = mrtPicard.texture[0];            // q1
          uniforms.textureSource2.value = mrtPicard.texture[1];            // q2
          renderer.setRenderTarget(simTextures[0]);
          renderer.render(simScene, simCamera);
      ...
      case "ADER4":
        ...
        refreshAderDirichletUniforms();
        refreshAderFluxConfig();
        // ADER-DG order 4, MRT-fused: 7 passes → 4 passes.
        //   Pass 1 (init MRT)        : u^n → q1^(1), q2^(1)   (mrtPredInit)
        //   Pass 2 (Picard sweep 1)  : u^n, q1^(1), q2^(1) → q1^(2), q2^(2)   (mrtPicard)
        //   Pass 3 (Picard sweep 2)  : u^n, q1^(2), q2^(2) → q1^(3), q2^(3)   (mrtPredInit, ping-pong)
        //   Pass 4 (corrector)       : u^n, q1^(3), q2^(3) → u^{n+1}          (simTextures[0])
        // Both Picard sweeps use the same hardcoded 2x2 weight matrix, so
        // the same shader serves both — only the input/output FBOs differ.
        {
          uniforms.dt.value = options.dt;

          // Pass 1: fused predictor init at GL2 nodes c1, c2.
          simDomain.material = simMaterials["ADER4initMRT"];
          uniforms.textureSource.value = simTextures[1].texture;
          renderer.setRenderTarget(mrtPredInit);
          renderer.render(simScene, simCamera);

          // Pass 2: Picard sweep 1, fused. Read q^(1) from mrtPredInit, write q^(2) to mrtPicard.
          simDomain.material = simMaterials["ADER4picMRT"];
          uniforms.textureSource.value  = simTextures[1].texture;
          uniforms.textureSource1.value = mrtPredInit.texture[0];   // q1^(1)
          uniforms.textureSource2.value = mrtPredInit.texture[1];   // q2^(1)
          renderer.setRenderTarget(mrtPicard);
          renderer.render(simScene, simCamera);

          // Pass 3: Picard sweep 2, fused. Read q^(2) from mrtPicard, write q^(3) back into mrtPredInit
          // (which we no longer need for q^(1)).
          uniforms.textureSource1.value = mrtPicard.texture[0];     // q1^(2)
          uniforms.textureSource2.value = mrtPicard.texture[1];     // q2^(2)
          renderer.setRenderTarget(mrtPredInit);
          renderer.render(simScene, simCamera);

          // Pass 4: corrector → u^{n+1}.
          simDomain.material = simMaterials["ADER4corr"];
          uniforms.textureSource.value  = simTextures[1].texture;
          uniforms.textureSource1.value = mrtPredInit.texture[0];   // q1^(3)
          uniforms.textureSource2.value = mrtPredInit.texture[1];   // q2^(3)
          renderer.setRenderTarget(simTextures[0]);
          renderer.render(simScene, simCamera);
      ...
      case "ADER1":
      case "ADER2":
        if (options.timesteppingScheme === "ADER1") {
          simDomain.material = simMaterials["FE"];
          uniforms.textureSource.value = simTextures[1].texture;
          uniforms.textureSource1.value = simTextures[2].texture;
          uniforms.dt.value = options.dt;
          ...
          simTextures.rotate(-1);
          uniforms.t.value += options.dt;
        } else if (options.timesteppingScheme === "ADER2") {
          simDomain.material = simMaterials["Mid1"];
          uniforms.textureSource.value = simTextures[1].texture;
          renderer.setRenderTarget(simTextures[2]);
          renderer.render(simScene, simCamera);
          ...
          // ADER3 and ADER4: route through RK4 as the closest 4th-order option.
          simDomain.material = simMaterials["RK41"];
          uniforms.textureSource.value = simTextures[1].texture;
          renderer.setRenderTarget(simTextures[2]);
          renderer.render(simScene, simCamera);
  ...
  // Build the DG-aware dgComputeRHS body from the user's reactionStr_*
  // expressions. Returns { kineticUniformsGLSL, body } that _aderHeader
  // expects.
  //
  // The existing parser (parseShaderString) emits GLSL referencing:
  //   uvwq.{rgba}     — current values of u/v/w/q at the node
  //   uvwqX.{rgba}    — first x-derivatives
  //   uvwqY.{rgba}    — first y-derivatives
  //   uvwqXX.{rgba}   — second x-derivatives
  //   uvwqYY.{rgba}   — second y-derivatives
  //
  // For the ADER-DG path we redefine these as locals inside dgComputeRHS:
  //   uvwq   ← s.self                                (current LGL node value)
  //   uvwqX  ← (2/h) · D · u-vector                  (LGL D matrix, x-direction)
  //   uvwqXX ← (4/h²) · D² · u-vector                (LGL D² matrix, x-direction)
  //   uvwqY  ← 4th-order central FD on y-row neighbours
  //   uvwqYY ← 4th-order central FD on y-row neighbours
  // The user's parsed reaction string then references these locals and
  // produces UFUN / VFUN / WFUN / QFUN, which we pack into the returned vec4.
  //
  // SUPPORTED: point-wise reactions, first and second x-derivatives via the
  //            DG D and D² matrices, first and second y-derivatives via the
  //            FD-4 stencil, kineticParams uniforms.
  // NOT yet:   texture lookups, custom random functions inside reactions.
  function buildDGComputeRHSBody(p) {
    const n = p + 1;
    const argsList = Array.from({ length: n }, (_, i) => `s.u${i}`).join(", ");
    // Use the existing parser to handle species substitutions, kineticParams
    // references, derivative tokens, etc. Parsed output uses uvwq, uvwqX, ...
    // which we redefine below.
    const ufun = parseShaderString(options.reactionStr_1);
    const vfun = parseShaderString(options.reactionStr_2);
    const wfun = parseShaderString(options.reactionStr_3);
    const qfun = parseShaderString(options.reactionStr_4);
    const kineticUniformsGLSL = kineticUniformsForShader();
    const body = `
  // First and second x-derivatives via the LGL D and D^(2) matrices, mapped
  // from reference space [-1,1] to the element by the chain rule
  //   d/dx = (2/h) d/dxi,    d²/dx² = (4/h²) d²/dxi².
  // y-derivatives use the standard 4th-order central FD stencil (the
  // hybrid 1D-DG-x + FD-y layout). In 1D problems the y-neighbours all
  // alias to s.self via texture wrap, so the y-stencils evaluate to zero.
  vec4 uvwq   = s.self;
  vec4 uvwqX  = (2.0 / u_dxElement) * dgddxi(s.nodeIdx, ${argsList});
  vec4 uvwqXX = (4.0 / (u_dxElement * u_dxElement)) * dgddxi2(s.nodeIdx, ${argsList});
  vec4 uvwqY  = (-s.yTT + 8.0 * s.yT - 8.0 * s.yB + s.yBB) / (12.0 * dy);
  vec4 uvwqYY = (-s.yTT + 16.0 * s.yT - 30.0 * s.self
                 + 16.0 * s.yB - s.yBB) / (12.0 * dy * dy);
  float UFUN = ${ufun};
  float VFUN = ${vfun};
  float WFUN = ${wfun};
  float QFUN = ${qfun};
  return vec4(UFUN, VFUN, WFUN, QFUN);`;
    return { kineticUniformsGLSL, body };
  }
  ...
  function setRDEquations() {
    ...
    // ADER-DG shaders. The element-local RHS (dgComputeRHS) is built from
    // the user's reactionStr_1..4 via the parser adapter buildDGComputeRHSBody
    // so the same pipeline solves any user-defined PDE the expression parser
    // supports: first and second x-derivatives via the LGL D and D² matrices,
    // y-derivatives via the standard 4th-order central FD stencil (the
    // hybrid 1D-DG-x + FD-y layout), point-wise reactions, kineticParams.
    //
    // Polynomial degree p is one lower than the scheme label so that visible
    // element-boundary structure on demos is reduced while the method order
    // min(k+1, 2p) is preserved (ADER3: p=2, k=2 → order 3; ADER4: p=3, k=3
    // → order 4). Spatial truncation O(h^(p+1)) matches the time order in
    // both cases.
    const P_ADER3 = 2;  // ADER3 runs DG at p=2 → 3 LGL nodes/elem.
    const P_ADER4 = 3;  // ADER4 runs DG at p=3 → 4 LGL nodes/elem.
    const aderRhsBody3 = buildDGComputeRHSBody(P_ADER3);
    const aderRhsBody4 = buildDGComputeRHSBody(P_ADER4);
    // ADER3 (p=2 → 3 LGL nodes/elem):
    assignFragmentShader(simMaterials["ADER3init1"], RDShaderDGPredictorInit(P_ADER3, aderRhsBody3));
    assignFragmentShader(simMaterials["ADER3init2"], RDShaderDGPredictorInit(P_ADER3, aderRhsBody3));
    assignFragmentShader(simMaterials["ADER3pic1"],  RDShaderDGPicard(P_ADER3,        aderRhsBody3));
    assignFragmentShader(simMaterials["ADER3pic2"],  RDShaderDGPicard(P_ADER3,        aderRhsBody3));
    assignFragmentShader(simMaterials["ADER3corr"],  RDShaderDGCorrector(P_ADER3,     aderRhsBody3));
    // ADER4 (p=3 → 4 LGL nodes/elem). Same shader generators, larger stencil.
    assignFragmentShader(simMaterials["ADER4init1"], RDShaderDGPredictorInit(P_ADER4, aderRhsBody4));
    assignFragmentShader(simMaterials["ADER4init2"], RDShaderDGPredictorInit(P_ADER4, aderRhsBody4));
    assignFragmentShader(simMaterials["ADER4pic1"],  RDShaderDGPicard(P_ADER4,        aderRhsBody4));
    assignFragmentShader(simMaterials["ADER4pic2"],  RDShaderDGPicard(P_ADER4,        aderRhsBody4));
    assignFragmentShader(simMaterials["ADER4corr"],  RDShaderDGCorrector(P_ADER4,     aderRhsBody4));
    // MRT-fused variants. Compiled in GLSL3 dialect via glslVersion on the
    // material; declare two `out vec4` outputs each.
    assignFragmentShader(simMaterials["ADER3initMRT"], RDShaderDGPredictorInitMRT(P_ADER3, aderRhsBody3));
    assignFragmentShader(simMaterials["ADER3picMRT"],  RDShaderDGPicardMRT(P_ADER3,        aderRhsBody3));
    assignFragmentShader(simMaterials["ADER4initMRT"], RDShaderDGPredictorInitMRT(P_ADER4, aderRhsBody4));
    assignFragmentShader(simMaterials["ADER4picMRT"],  RDShaderDGPicardMRT(P_ADER4,        aderRhsBody4));
  ...
  function configureManualInterpolation() {
    // ADER-DG stores values at LGL-spaced nodes (clustered at element
    // endpoints), so a NEAREST filter on a uniform-coordinate display
    // shows the clustering as a visible regular cell pattern. Force
    // LinearFilter when DG is active — the bilinear interpolation between
    // texels hides the clustering without changing the simulation values.
    const dgActive = aderDgOrder() >= 2;
    if (isManuallyInterpolating() && !dgActive) {
      simTextures.forEach((simTex) => {
